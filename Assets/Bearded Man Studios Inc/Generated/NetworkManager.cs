...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] JugadorNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is JugadorNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (JugadorNetworkObject.Length > 0 && JugadorNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(JugadorNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<JugadorBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateJugador instead, its shorter and easier to type out ;)")]
		public JugadorBehavior InstantiateJugadorNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(JugadorNetworkObject[index]);
			var netBehavior = go.GetComponent<JugadorBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<JugadorBehavior>().networkObject = (JugadorNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Jugador
		/// </summary>
		/// <returns>
		/// A local instance of JugadorBehavior
		/// </returns>
		/// <param name="index">The index of the Jugador prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public JugadorBehavior InstantiateJugador(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(JugadorNetworkObject[index]);
			var netBehavior = go.GetComponent<JugadorBehavior>();
